...
Patch0: setuserid-fix.patch
Patch1: root-epilog-fix.patch
Buildroot: %{_tmppath}/%{name}-%{version}-buildroot
# BuildRequires: libpng-devel, libart_lgpl-devel, gcc-c++, python-devel, libconfuse-devel, make, pcre-devel, autoconf, automake, libtool, pkgconfig, libmemcached-last-devel, protobuf-c-devel
%if 0%{?suse_version}
...
%endif
%else
BuildRequires: rrdtool-devel, apr-devel > 1, libconfuse-devel, expat-devel
%endif
BuildRequires: libmemcached-last-devel >= 1.0.2, pcre-devel, zlib-devel, python-devel, autoconf, automake, libtool, pkgconfig
...
Requires: ganglia-libs = %{version}-%{release}
...
%if 0%{?suse_version}
Requires: libconfuse-devel, libapr1-devel, ganglia-libs = %{version}-%{release}
%if 0%{?suse_version} > 1020
...
sed -c -i '/user = nobody/s/user = nobody/user = ganglia/' gmond/gmond.conf.html gmond/conf.pod gmond/gmond.conf.5

%build
%configure --sysconfdir=%{conf_dir} --enable-setuid="ganglia" --enable-setgid="ganglia" \
	--with-gmetad --with-memcached
%ifnarch noarch
%__make CPPFLAGS=-DREMOVE_BOGUS_SPIKES
%endif
#cd gmetad-python
...
%post   -n ganglia-libs -p /sbin/ldconfig

%postun -n ganglia-libs -p /sbin/ldconfig
...
%files gmetad
%defattr(-,root,root)
%attr(0755,ganglia,ganglia)/var/lib/ganglia/
%{_sbindir}/gmetad
%if 0%{?_unitdir:1}
...
%{_mandir}/man1/gmetad.1.gz
%attr(0600,ganglia,ganglia) %config(noreplace) %{conf_dir}/gmetad.conf
...
%attr(0600,ganglia,ganglia) %config(noreplace) %{conf_dir}/gmond.conf
%dir %{conf_dir}
%dir %{conf_dir}/conf.d/
%config(noreplace) %{conf_dir}/conf.d/modpython.conf
%exclude /etc/ganglia/conf.d/modgstatus.conf
%dir %{_libdir}/ganglia/
...
%exclude /usr/share/man/man1/gmetad.py.1.gz
...
- rebuild with rrdtool 1.4.8
* Mon Nov 27 2017 Xiangfei Liang <liangxiangfei@domob.cn>
- rebuild for Domob.Inc
- FIX: unable to write root epilog
