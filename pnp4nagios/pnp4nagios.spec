...
sed -e 's|/usr/local/nagios/etc/htpasswd.users|/etc/icinga/passwd|' \
    -e '/AuthName/s/Nagios/Icinga/' \
...
mv $RPM_BUILD_ROOT%{_libdir}/kohana \
  $RPM_BUILD_ROOT%{_datadir}/%{name}/kohana
sed -i 's|%{_libdir}/kohana|%{_datadir}/%{name}/kohana|' \
  $RPM_BUILD_ROOT%{_datadir}/%{name}/index.php
...
%config(noreplace) %attr(0640,root,icinga) %{_sysconfdir}/pnp4nagios/process_perfdata.cfg
...
%exclude %{_libdir}/npcdmod.o
%dir %{_libexecdir}/%{name}
%attr(755,root,root) %{_libexecdir}/%{name}/*
%attr(755,icinga,icinga) %{_localstatedir}/lib/%{name}
%attr(755,icinga,icinga) %{_localstatedir}/log/%{name}
%attr(755,icinga,icinga) %{_localstatedir}/spool/%{name}
%{_datadir}/%{name}
# Remove install check script
# as it is not required if all dependencies are met.
%exclude %{_datadir}/%{name}/install.php
%{_mandir}/man8/*
# This is a different version of Kohana like in Fedora/EPEL.
# Needed for pnp4nagios web interface to work.
%{_datadir}/%{name}/kohana
